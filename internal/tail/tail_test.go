package tail

import (
	"encoding/json"
	"os"
	"testing"

	"github.com/tiny-oc/toc/internal/runtime"
)

func TestReadNewLines_CompleteLines(t *testing.T) {
	provider := testProvider(t)
	path := writeJSONLFile(t, []map[string]interface{}{
		assistantMsg("Read", map[string]string{"file_path": "a.go"}),
		assistantMsg("Bash", map[string]string{"command": "go test"}),
	})

	events, offset, partial := readNewLines(path, 0, nil, provider)
	if len(events) != 2 {
		t.Fatalf("got %d events, want 2", len(events))
	}
	if events[0].Step.Tool != "Read" || events[0].Step.Path != "a.go" {
		t.Errorf("event 0 = %+v, want Read a.go", events[0])
	}
	if events[1].Step.Tool != "Bash" || events[1].Step.Command != "go test" {
		t.Errorf("event 1 = %+v, want Bash 'go test'", events[1])
	}
	if offset == 0 {
		t.Error("offset should advance past data")
	}
	if len(partial) != 0 {
		t.Errorf("partial = %q, want empty", partial)
	}
}

func TestReadNewLines_PartialLine(t *testing.T) {
	provider := testProvider(t)
	path := tempJSONLPath(t)

	// Write a complete line followed by a partial line (no trailing newline)
	line1 := marshalJSON(assistantMsg("Read", map[string]string{"file_path": "a.go"}))
	partial2 := `{"type":"assistant","message":{"role":"assistant","content":[{"type":"tool_use","name":"Edit"`

	os.WriteFile(path, []byte(line1+"\n"+partial2), 0644)

	events, offset, partial := readNewLines(path, 0, nil, provider)
	if len(events) != 1 {
		t.Fatalf("got %d events, want 1 (partial should be buffered)", len(events))
	}
	if events[0].Step.Tool != "Read" {
		t.Errorf("event 0 = %+v, want Read", events[0])
	}
	if string(partial) != partial2 {
		t.Errorf("partial = %q, want %q", partial, partial2)
	}

	// Now complete the partial line in a second read
	rest := `,"input":{"file_path":"b.go","old_string":"old","new_string":"new"}}]}}` + "\n"
	f, _ := os.OpenFile(path, os.O_APPEND|os.O_WRONLY, 0644)
	f.WriteString(rest)
	f.Close()

	events, _, partial = readNewLines(path, offset, partial, provider)
	if len(events) != 1 {
		t.Fatalf("got %d events, want 1 (completed partial)", len(events))
	}
	if events[0].Step.Tool != "Edit" || events[0].Step.Path != "b.go" {
		t.Errorf("event 0 = %+v, want Edit b.go", events[0])
	}
	if len(partial) != 0 {
		t.Errorf("partial = %q, want empty after completion", partial)
	}
}

func TestReadNewLines_EmptyRead(t *testing.T) {
	provider := testProvider(t)
	path := tempJSONLPath(t)
	os.WriteFile(path, []byte(""), 0644)

	events, offset, partial := readNewLines(path, 0, nil, provider)
	if len(events) != 0 {
		t.Errorf("got %d events, want 0", len(events))
	}
	if offset != 0 {
		t.Errorf("offset = %d, want 0", offset)
	}
	if len(partial) != 0 {
		t.Errorf("partial = %q, want empty", partial)
	}
}

func TestReadNewLines_PrependPartialBuffer(t *testing.T) {
	provider := testProvider(t)
	path := tempJSONLPath(t)

	// Previous read left a partial buffer; new data completes it
	full := marshalJSON(assistantMsg("Bash", map[string]string{"command": "ls"}))
	mid := len(full) / 2
	partialBuf := []byte(full[:mid])
	remainder := full[mid:] + "\n"

	os.WriteFile(path, []byte(remainder), 0644)

	events, _, partial := readNewLines(path, 0, partialBuf, provider)
	if len(events) != 1 {
		t.Fatalf("got %d events, want 1", len(events))
	}
	if events[0].Step.Tool != "Bash" || events[0].Step.Command != "ls" {
		t.Errorf("event 0 = %+v, want Bash 'ls'", events[0])
	}
	if len(partial) != 0 {
		t.Errorf("partial = %q, want empty", partial)
	}
}

func TestReadNewLines_IncludesUserMessage(t *testing.T) {
	provider := testProvider(t)
	path := tempJSONLPath(t)

	userLine := marshalJSON(map[string]interface{}{
		"type":    "user",
		"message": map[string]interface{}{"role": "user", "content": "hello"},
	})
	assistLine := marshalJSON(assistantMsg("Read", map[string]string{"file_path": "x.go"}))

	os.WriteFile(path, []byte(userLine+"\n"+assistLine+"\n"), 0644)

	events, _, _ := readNewLines(path, 0, nil, provider)
	if len(events) != 2 {
		t.Fatalf("got %d events, want 2", len(events))
	}
	if events[0].Step.Type != "user" || events[0].Step.Content != "hello" {
		t.Errorf("event 0 = %+v, want user 'hello'", events[0])
	}
	if events[1].Step.Tool != "Read" {
		t.Errorf("event 1 = %+v, want Read", events[1])
	}
}

func TestReadNewLines_SkipsNonMessageTypes(t *testing.T) {
	provider := testProvider(t)
	path := tempJSONLPath(t)

	systemLine := marshalJSON(map[string]interface{}{
		"type":    "system",
		"message": map[string]interface{}{"role": "system", "content": "system prompt"},
	})
	assistLine := marshalJSON(assistantMsg("Read", map[string]string{"file_path": "x.go"}))

	os.WriteFile(path, []byte(systemLine+"\n"+assistLine+"\n"), 0644)

	events, _, _ := readNewLines(path, 0, nil, provider)
	if len(events) != 1 {
		t.Fatalf("got %d events, want 1 (system message should be skipped)", len(events))
	}
	if events[0].Step.Tool != "Read" {
		t.Errorf("event 0 = %+v, want Read", events[0])
	}
}

// --- helpers ---

func tempJSONLPath(t *testing.T) string {
	t.Helper()
	return t.TempDir() + "/test.jsonl"
}

func assistantMsg(tool string, input map[string]string) map[string]interface{} {
	return map[string]interface{}{
		"type": "assistant",
		"message": map[string]interface{}{
			"role": "assistant",
			"content": []map[string]interface{}{
				{"type": "tool_use", "name": tool, "input": input},
			},
		},
	}
}

func marshalJSON(v interface{}) string {
	b, _ := json.Marshal(v)
	return string(b)
}

func writeJSONLFile(t *testing.T, entries []map[string]interface{}) string {
	t.Helper()
	path := tempJSONLPath(t)
	var content string
	for _, e := range entries {
		content += marshalJSON(e) + "\n"
	}
	if err := os.WriteFile(path, []byte(content), 0644); err != nil {
		t.Fatal(err)
	}
	return path
}

func testProvider(t *testing.T) runtime.Provider {
	t.Helper()
	provider, err := runtime.Get(runtime.DefaultRuntime)
	if err != nil {
		t.Fatal(err)
	}
	return provider
}
